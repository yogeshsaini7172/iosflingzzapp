  ...
  try {
    ...
    switch(action){
      case 'list':
        {
          ...
          const userIds = Array.from(new Set((threadsData || []).map((t)=>t.user_id).filter(Boolean)));
          // Fetch replies for all threads
          const threadIds = (threadsData || []).map((t)=>t.id);
          let repliesByThreadId = {};
          if (threadIds.length > 0) {
            const { data: repliesData, error: repliesError } = await supabase.from('thread_replies').select('*').in('thread_id', threadIds).order('created_at', {
              ascending: true
            });
            ...
            repliesByThreadId = (repliesData || []).reduce((acc, reply)=>{
          ...
          if (uniqueUserIds.length > 0) {
            const { data: profiles, error: profilesError } = await supabase.from('profiles').select('user_id, first_name, profile_images').in('user_id', uniqueUserIds);
          ...
          const formatted = (threadsData || []).map((t)=>{
            ...
            return {
              ...
              replies: (repliesByThreadId[t.id] || []).map((reply)=>({
                  ...reply,
      ...
      case 'update':
        {
          ...
          const { data: updatedThread, error: updateError } = await supabase.from('threads').update({
            content: content.trim(),
            updated_at: new Date().toISOString()
          }).eq('id', threadId).eq('user_id', userId).select().single();
      ...
      case 'like':
        ...
        const { error: likeError } = await supabase.from('thread_likes').insert({
          thread_id: threadId,
          user_id: userId
        });
      ...
      case 'unlike':
        ...
        const { error: unlikeError } = await supabase.from('thread_likes').delete().eq('thread_id', threadId).eq('user_id', userId);
      ...
      case 'reply':
        ...
        const { data: newReply, error: replyError } = await supabase.from('thread_replies').insert({
          thread_id: threadId,
          user_id: userId,
          content: content.trim()
        }).select().single();
