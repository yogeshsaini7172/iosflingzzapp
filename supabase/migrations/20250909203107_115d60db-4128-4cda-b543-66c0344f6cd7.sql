-- Fix the ambiguous column reference and syntax in rpc_accept_chat_request
...
BEGIN
  ...
  INSERT INTO chat_rooms (match_id, user1_id, user2_id, created_at)
    ...
    RETURNING cr2.id INTO inserted_chat_id;

  IF inserted_chat_id IS NULL THEN
    SELECT cr2.id INTO inserted_chat_id FROM chat_rooms cr2 WHERE cr2.match_id = inserted_match_id LIMIT 1;
  END IF;
