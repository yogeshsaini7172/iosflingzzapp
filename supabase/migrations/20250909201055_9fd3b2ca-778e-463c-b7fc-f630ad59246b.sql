-- Add unique constraint to prevent duplicate enhanced_matches
...
-- Add unique constraint to prevent duplicate chat rooms per match
ALTER TABLE chat_rooms ADD CONSTRAINT ux_chat_rooms_match_id UNIQUE (match_id);

-- Create atomic function to handle chat request acceptance
CREATE OR REPLACE FUNCTION rpc_accept_chat_request(
  p_chat_request_id uuid, 
  p_recipient_id text
)
RETURNS TABLE(match_id uuid, chat_room_id uuid, created_match boolean) AS $$
DECLARE
  cr record;
  u1 text;
  u2 text;
...
BEGIN
  ...
  ELSE
    ...
    SELECT id INTO inserted_match_id
      FROM enhanced_matches
      WHERE LEAST(user1_id, user2_id) = u1 AND GREATEST(user1_id, user2_id) = u2
      LIMIT 1;
  END IF;
  ...
  INSERT INTO chat_rooms (match_id, user1_id, user2_id, created_at)
    ...
    RETURNING id INTO inserted_chat_id;

  IF inserted_chat_id IS NULL THEN
    SELECT id INTO inserted_chat_id FROM chat_rooms WHERE match_id = inserted_match_id LIMIT 1;
  END IF;
