-- Resolve ambiguity in ON CONFLICT by referencing the unique constraint explicitly
...
BEGIN
  ...
  INSERT INTO chat_rooms (match_id, user1_id, user2_id, created_at)
    ...
    ON CONFLICT ON CONSTRAINT chat_rooms_match_id_key DO NOTHING
    RETURNING id INTO inserted_chat_id;

  IF inserted_chat_id IS NULL THEN
    SELECT cr_table.id INTO inserted_chat_id 
    FROM chat_rooms cr_table 
    WHERE cr_table.match_id = inserted_match_id 
    LIMIT 1;
  END IF;
