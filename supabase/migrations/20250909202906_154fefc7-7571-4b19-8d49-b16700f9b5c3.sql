-- Fix the ambiguous column reference in rpc_accept_chat_request
...
BEGIN
  ...
  ELSE
    ...
    SELECT enhanced_matches.id INTO inserted_match_id
      FROM enhanced_matches
      WHERE LEAST(enhanced_matches.user1_id, enhanced_matches.user2_id) = u1 AND GREATEST(enhanced_matches.user1_id, enhanced_matches.user2_id) = u2
      LIMIT 1;
  END IF;
  ...
  INSERT INTO chat_rooms (match_id, user1_id, user2_id, created_at)
    ...
    ON CONFLICT (chat_rooms.match_id) DO NOTHING
